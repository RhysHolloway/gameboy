...
pub struct Pixels<'win> {
    ...
    scaling_matrix_inverse: glam::Mat4,
}
...
impl core::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Error::AdapterNotFound(e) => write!(f, "No suitable `wgpu::Adapter` found, {e}"),
            Error::DeviceNotFound(e) => write!(f, "No wgpu::Device found, {e}"),
            Error::Surface(e) => write!(f, "The GPU failed to acquire a surface frame, {e}"),
            Error::CreateSurface(e) => write!(f, "Unable to create a surface, {e}"),
            Error::InvalidTexture(e) => write!(f, "Texture creation failed: {e}"),
...
impl core::fmt::Display for TextureError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            TextureError::TextureWidth(w) => write!(f, "Texture width is invalid: {w}"),
            TextureError::TextureHeight(h) => write!(f, "Texture height is invalid: {h}"),
        }
    }
}
